...
u.transformExpected = function(h) {
  var i, key, len;
...
equal(`LOG someSymbol`, {
  lUsed: words('LOG someSymbol'),
  lMissing: words('LOG someSymbol')
});

equal(`import {toArray, toBlock} from '@jdeighan/coffee-utils'
...
equal(`export toBlock = (lItems) ->
	return lItems.join("\n")`, {
  lExported: ['toBlock'],
  lUsed: ['lItems']
});

equal(`export meaning = 42`, {
  lExported: ['meaning']
});

equal(`import {undef} from '@jdeighan/coffee-utils'
x = undef`, {
  lImported: ['undef'],
  lUsed: ['undef']
});
...
equal(`import {undef, toArray} from '@jdeighan/coffee-utils'
x = toArray("abc")`, {
  lUsed: ['toArray'],
...
equal(`export isHashComment = (line) =>
	return defined(line)`, {
...
equal(`export isSubclassOf = (subClass, superClass) ->
...
equal(`export patchStr = (bigstr, pos, str) ->
...
equal(`export removeKeys = (h, lKeys) =>
